...
```json
{
  ...
  "fields": [
    ...
    },
    {
      ...
      "usage": "给前端时间线或 Agent 编排层按 stage/status/message/nextAction 展示执行过程",
      "displayOrder": 80,
      "category": "trace",
      "uiLabel": "结构化分析轨迹",
      ...
      "fallbackValue": [
        {"stage": "route", "status": "NEEDS_INPUT", "message": "请重新运行 /api-docs/analyze 获取结构化轨迹。", "nextAction": "补充有效 OpenAPI/Swagger JSON。"}
      ],
      "validationRule": "应为包含 stage、status、message、nextAction 的非空对象列表；缺失/为空时使用 fallbackValue。",
      ...
      "readyExampleValue": [
        {"stage": "parse", "status": "DONE", "message": "识别接口 2 个。", "nextAction": "检查接口解析结果。"},
        {"stage": "route", "status": "READY", "message": "workflowStatus=READY，suggestedTool=api-risk-reviewer。", "nextAction": "进入 API 风险审查。"}
      ],
      "needsInputExampleValue": [
        {"stage": "parse", "status": "DONE", "message": "识别接口 0 个。", "nextAction": "检查接口解析结果。"},
        {"stage": "route", "status": "NEEDS_INPUT", "message": "workflowStatus=NEEDS_INPUT，suggestedTool=openapi-input-validator。", "nextAction": "补充有效 OpenAPI/Swagger JSON。"}
      ]
    }
  ]
}
...
```json
{
  ...
  "analysisTraceItems": [
    {"stage": "parse", "status": "DONE", "message": "识别接口 2 个。", "nextAction": "检查接口解析结果。"},
    {"stage": "aggregate", "status": "DONE", "message": "聚合模块 2 个。", "nextAction": "查看模块聚合结果。"},
    {"stage": "prioritize", "status": "DONE", "message": "生成审查步骤 2 个。", "nextAction": "按审查优先级执行。"},
    {"stage": "route", "status": "READY", "message": "workflowStatus=READY，suggestedTool=api-risk-reviewer。", "nextAction": "进入 API 风险审查。"},
    {"stage": "advise", "status": "DONE", "message": "生成风险提示和测试建议 2 条。", "nextAction": "查看风险提示和测试建议。"}
  ]
}
```
...
```json
{
  ...
  "analysisTraceItems": [
    {"stage": "parse", "status": "DONE", "message": "识别接口 0 个。", "nextAction": "检查接口解析结果。"},
    {"stage": "aggregate", "status": "DONE", "message": "聚合模块 0 个。", "nextAction": "查看模块聚合结果。"},
    {"stage": "prioritize", "status": "DONE", "message": "生成审查步骤 0 个。", "nextAction": "按审查优先级执行。"},
    {"stage": "route", "status": "NEEDS_INPUT", "message": "workflowStatus=NEEDS_INPUT，suggestedTool=openapi-input-validator。", "nextAction": "补充有效 OpenAPI/Swagger JSON。"},
    {"stage": "advise", "status": "DONE", "message": "生成风险提示和测试建议 0 条。", "nextAction": "查看风险提示和测试建议。"}
  ]
}
```
