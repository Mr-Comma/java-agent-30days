    ...
    void generatesRiskHintsAndTestSuggestionsForParsedEndpoints() {
        ...
        assertThat(response.reviewPromptVariables())
                .isEqualTo(new ReviewPromptVariables(
                        ...
                        "先审查删除接口、权限控制和误删保护。",
                        "请输出风险说明、测试建议和下一步行动。"));
        assertThat(response.reviewPromptPreview())
                .isEqualTo("请调用 api-risk-reviewer 审查 orders 模块：先审查删除接口、权限控制和误删保护；请输出风险说明、测试建议和下一步行动。");
        assertThat(response.debugHints())
                .containsExactly(
        ...
        assertThat(response.analysisTraceItems())
                .containsExactly(
                        new ApiAnalysisTraceItem("parse", "DONE", "识别接口 4 个。", "检查接口解析结果。", "INSPECT_PARSED_ENDPOINTS"),
                        new ApiAnalysisTraceItem("aggregate", "DONE", "聚合模块 3 个。", "查看模块聚合结果。", "REVIEW_MODULE_SUMMARY"),
                        new ApiAnalysisTraceItem("prioritize", "DONE", "生成审查步骤 3 个。", "按审查优先级执行。", "EXECUTE_REVIEW_PRIORITY"),
                        new ApiAnalysisTraceItem("route", "READY", "workflowStatus=READY，suggestedTool=api-risk-reviewer。",
                                "进入 API 风险审查。", "START_API_RISK_REVIEW"),
                        new ApiAnalysisTraceItem("advise", "DONE", "生成风险提示和测试建议 4 条。", "查看风险提示和测试建议。", "REVIEW_RISK_AND_TEST_ADVICE"));
    ...
    void returnsEmptySummaryWhenNoEndpointsParsed() {
        ...
        assertThat(response.reviewPromptVariables())
                .isEqualTo(new ReviewPromptVariables(
                        ...
                        "请先补充有效输入，不要编造接口。"));
        assertThat(response.reviewPromptPreview())
                .isEqualTo("请调用 openapi-input-validator 处理 INPUT_REQUIRED 阶段：OpenAPI/Swagger JSON 缺少 paths 或未解析到接口；请先补充有效输入，不要编造接口。");
        assertThat(response.debugHints())
                .containsExactly(
        ...
        assertThat(response.analysisTraceItems())
                .containsExactly(
                        new ApiAnalysisTraceItem("parse", "DONE", "识别接口 0 个。", "检查接口解析结果。", "INSPECT_PARSED_ENDPOINTS"),
                        new ApiAnalysisTraceItem("aggregate", "DONE", "聚合模块 0 个。", "查看模块聚合结果。", "REVIEW_MODULE_SUMMARY"),
                        new ApiAnalysisTraceItem("prioritize", "DONE", "生成审查步骤 0 个。", "按审查优先级执行。", "EXECUTE_REVIEW_PRIORITY"),
                        ...
                        new ApiAnalysisTraceItem("advise", "DONE", "生成风险提示和测试建议 0 条。", "查看风险提示和测试建议。", "REVIEW_RISK_AND_TEST_ADVICE"));
