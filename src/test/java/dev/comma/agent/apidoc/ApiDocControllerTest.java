...
class ApiDocControllerTest {
    ...
    void exposesInputBlockingDebugFieldsFromAnalyzeEndpoint() throws Exception {
        ...
        mockMvc.perform(post("/api-docs/analyze")
                ...
                .andExpect(jsonPath("$.toolCallPlan.tool").value("openapi-input-validator"))
                .andExpect(jsonPath("$.toolCallPlan.reason").value("校验 OpenAPI/Swagger JSON 并补齐可解析的 paths。"))
                .andExpect(jsonPath("$.toolCallPlan.payloadKeys[0]").value("blockingReason"))
    ...
    void exposesReviewPromptDebugFieldsFromAnalyzeEndpoint() throws Exception {
        ...
        mockMvc.perform(post("/api-docs/analyze")
                ...
                .andExpect(jsonPath("$.toolCallPlan.tool").value("api-risk-reviewer"))
                .andExpect(jsonPath("$.toolCallPlan.reason").value("按首要模块执行 API 风险审查。"))
                .andExpect(jsonPath("$.toolCallPlan.payloadKeys[0]").value("topPriorityModule"))
                .andExpect(jsonPath("$.toolCallPlan.payloadKeys[1]").value("reviewPromptVariables"))
