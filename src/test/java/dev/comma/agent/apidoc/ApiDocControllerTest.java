...
class ApiDocControllerTest {
    ...
    void exposesApiDocDebugSchemaForFrontendAndAgentRouting() throws Exception {
        mockMvc.perform(get("/api-docs/debug-schema"))
                ...
                .andExpect(jsonPath("$.workflowStatusAllowedValues[0]").value("READY"))
                .andExpect(jsonPath("$.workflowStatusAllowedValues[1]").value("NEEDS_INPUT"))
                .andExpect(jsonPath("$.workflowStageAllowedValues[0]").value("REVIEW_READY"))
                .andExpect(jsonPath("$.workflowStageAllowedValues[1]").value("INPUT_REQUIRED"))
                .andExpect(jsonPath("$.suggestedToolAllowedValues[0]").value("api-risk-reviewer"))
                .andExpect(jsonPath("$.suggestedToolAllowedValues[1]").value("openapi-input-validator"))
