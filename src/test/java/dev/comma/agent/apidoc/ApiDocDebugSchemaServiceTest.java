...
class ApiDocDebugSchemaServiceTest {
    ...
    void returnsStableDebugFieldSchemaForWorkflowRouting() {
        ...
        assertThat(response.fields())
                ...
                .containsExactly(
                        ...
                        "当 workflowStage 缺失、非法或持续为 INPUT_REQUIRED 且无法进入审查阶段时升级。",
                        "无需升级；缺失或非法时使用 fallbackValue 继续工具分发。",
        ...
        assertThat(response.fields())
                ...
                .containsExactly(
                        ...
                        "已将阶段问题升级给 openapi-input-owner：请确认输入可进入审查阶段。",
                        "无需升级：已降级到 openapi-input-validator 并可继续分发。",
    ...
    private Map<String, Object> traceItem(String stage, String status, String message, String nextAction, String nextActionCode) {
        Map<String, Object> values = new LinkedHashMap<>();
